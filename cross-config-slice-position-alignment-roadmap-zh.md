...
```text
Position_X = (build_id_X, anchor_id_X, occurrence_X, event_phase)
Correspondence(A_event, B_event)
```

`occurrence_X` 是 build X 内的局部计数，不假设 A/B 数值相等；对应关系可以包含 occurrence transform，用来表达循环展开、向量化或插入/删除事件。`event_phase` 明确是 `before_instruction`、`after_taken_edge` 或其他精确定义的触发时刻。B 中的 PC、workload icount 和 interval index 都是这个语义位置在配置 B 下的物化结果，不是跨配置身份。
...
1. B 内精确位置：`(anchor_id_B, occurrence_B, event_phase)`，以及 A/B 事件对应证据；
...
当前 [通用脚本](experiment/cross_elf_checkpoint.py) 已经具备 `prepare -> align -> checkpoint -> report` 骨架。现有方法以总指令数比例确定搜索中心，在 `+/-8` 个 B interval 内比较 BBV 动态 count multiset，并加入 `+/-2` interval 的同 offset 上下文。
...
| 索引式 | `build_index(...)` + `align(...)` | 同一运行证据可复用；manifest 绑定清楚；适合 A->B/C/D | 比一键式多一个概念 | 推荐的公开 interface |
...
index = build_index(
    ...
    policy_version="calibrated-default",
)
...
- `event_trace`：动态 `(anchor_id, occurrence, event_phase, workload_icount, context)`；
...
1. 在 held-out trace 中核对 B1 是否命中 gold `(anchor_id_B, occurrence_B, event_phase)`；
2. 核对 B1 前后的独立 source/IR marker 序列，不使用同一 BBV score 自证；
3. 从头运行 B，确认目标事件唯一可达，运行输出与 manifest 一致；
4. exact 的 occurrence/event phase 必须完全一致；snapped 的 A 侧位移必须不超过冻结 policy 的上限；
...
- 实现推荐的 `build_index` / `align` interface；
- BBV 作为第一个 EvidenceAdapter，保留当前结果作回归基线；
...
- RunnerAdapter 支持 `(anchor_id, occurrence, event_phase)` 触发；
...
- trigger 精确命中 `(anchor_id_B, occurrence_B, event_phase)`；
