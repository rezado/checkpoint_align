...
from checkpoint_align.cli import bind_source_position, command_calibrate_source_checkpoints, command_checkpoint_all, command_checkpoint_suite, command_materialize
...
class MaterializeCommandTest(unittest.TestCase):
    def test_zero_runner_exit_without_checkpoint_is_not_success(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            alignment.write_text(json.dumps({"correspondence": {"target": {"build_id": "B", "anchor_id": "main", "occurrence": 0, "pc": 10}}}))
...
class CheckpointBatchCommandTest(unittest.TestCase):
    ...
    def test_exact_boundary_request_writes_sidecar_from_actual_hit(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            materialized = {"exit_status": 0, "targets": {"7": {"hit": {"occurrence": 3, "pc": 0x2000, "workload_icount": 123}, "checkpoint": str(target_checkpoint), "checkpoint_sha256": "f" * 64}}}
    ...
    def test_resume_reuses_existing_sidecar(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            requests = root / "requests.json"; requests.write_text(json.dumps({"checkpoints": [{"id": "point-0", "request": {"anchor_id": "A:main", "occurrence": 0, "occurrence_scope": "global_from_scratch", "requested_icount": 100, "interval_instructions": 100}}]}))
            output = root / "out"
            ...
            with patch("checkpoint_align.cli.run_nemu_target", return_value={"exit_status": 0, "checkpoint_sidecar": str(output / "point-0" / "materialization" / "checkpoint-sidecar.json")}):
                ...
                sidecar = {"build_id": "B", "anchor_id": "B:main", "occurrence": 0, "pc": 110, "checkpoint": str(checkpoint)}
    ...
    def test_checkpoint_suite_passes_rng_seed_to_calibration(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            args = argparse.Namespace(suite=suite, workload_id="mcf", source_label="A", target_label="B", source_checkpoints=source_checkpoints, nemu=nemu, output_dir=root / "out", anchor_name=[], interval_instructions=20, warmup_instructions=20, boot_allowance=200, tail_instructions=20, max_instructions=100, max_snap_instructions=20, force_probe=False, rng_seed="1" * 64, timeout=10, plan_only=True, resume=False)
            captured = {}
