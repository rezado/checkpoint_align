...
from checkpoint_align.position_aligner import BuildRun, Position, PositionAligner, ProgressEvent, validate_events, validate_position, validate_run
from checkpoint_align.position_aligner.materialize import write_target
...
def event(build: str, anchor: str, occurrence: int, icount: int, semantic: str | None = None) -> ProgressEvent:
    return ProgressEvent(build, f"fixture-{build}-run1", f"{build}:{anchor}", semantic or anchor, occurrence, 0x1000 + icount, icount, {})
...
def position(value: ProgressEvent, requested: int | None = None) -> Position:
    ...
    return Position(value.build_id, value.anchor_id, value.occurrence, value.pc, value.workload_icount, interval, offset, requested, value.workload_icount - requested)
...
class ProtocolTest(unittest.TestCase):
    def test_build_run_rejects_weight_and_wrong_phase(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "event phase"):
            BuildRun(**{**run("A").__dict__, "event_phase": "after_instruction"})
    ...
    def test_progress_event_may_omit_qemu_icount(self) -> None:
        value = ProgressEvent("A", "fixture-A-run1", "A:loop", "loop", 0, 0x1000, None)
        self.assertIsNone(value.workload_icount)

    def test_phase_is_run_level_only(self) -> None:
        event_value = event("A", "loop", 0, 100).to_dict()
        position_value = position(event("A", "loop", 0, 100)).to_dict()
        self.assertNotIn("event_phase", event_value)
        self.assertNotIn("event_phase", position_value)
        with self.assertRaisesRegex(ValueError, "unknown ProgressEvent fields"):
            ProgressEvent.from_dict({**event_value, "event_phase": "before_instruction"})
    ...
    def test_target_requires_semantic_identity(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            with self.assertRaisesRegex(ValueError, "pc"):
                write_target(Path(directory) / "target.json", {"build_id": "B", "anchor_id": "B:loop", "occurrence": 0}, run_manifest=run("B").to_dict())
