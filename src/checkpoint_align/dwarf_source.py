    ...
    for line in text.splitlines():
        ...
        if "end_sequence" in flags:
            if len(rows) >= max_entries and pc not in rows:
                truncated = True
                continue
            rows.setdefault(pc, {"pc": pc, "source": None, "end_sequence": True})
            continue
        ...
        # Multiple rows can describe one instruction.  The first source row is
        # the producer's deterministic choice, but a new sequence may start at
        # the same address where the preceding sequence ended.
        if pc not in rows or rows[pc].get("end_sequence"):
            rows[pc] = candidate
...
class OccurrenceEvent:
    ...
    def from_dict(cls, value: Mapping[str, Any]) -> "OccurrenceEvent":
        if "event_phase" in value:
            raise ValueError("event_phase belongs to the occurrence trace, not an event")
        return cls(
            anchor_id=str(value["anchor_id"]),
...
class OccurrenceTrace:
    ...
    def __post_init__(self) -> None:
        if self.event_phase != "before_instruction":
            raise ValueError("only before_instruction occurrence traces are supported")
...
def align_occurrence_sequences(
    ...
    *,
    ...
    canonical_source = tuple((event.semantic_key, event.occurrence) for event in source)
    canonical_target = tuple((event.semantic_key, event.occurrence) for event in target)
    if (
        source
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    suite.add_argument("--suite", type=Path, default=Path("data/multi-workload"))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "collect":
        ...
        trace = collect_occurrence_trace(catalog, samples, mode=args.mode, source=str(args.trace))
