...
occurrence-sequence alignment primitive for the M2 roadmap milestone.  It is
kept separate from `cross_elf_checkpoint.py`; it does not launch NEMU/QEMU,
...
```sh
python3 experiment/dwarf_source.py catalog \
  --elf experiment/multi-workload/workloads/lbm/A/elf/lbm.elf \
  --manifest experiment/multi-workload/suite-manifest.json \
...
```sh
python3 experiment/dwarf_source.py catalog-suite \
  --suite experiment/multi-workload \
  --output experiment/dwarf-source/catalog-summary.json \
  --no-loops
```
...
`anchor_id`, zero-based local occurrence counter, explicit `event_phase`, optional PC and
workload icount, and a semantic key.  Unmatched samples are retained in the
trace metadata.

```sh
python3 experiment/dwarf_source.py collect \
...
```sh
python3 experiment/dwarf_source.py align \
...
```sh
python3 experiment/dwarf_source.py fixture \
  --output experiment/dwarf-source/controlled-positive.json
python3 - <<'PY'
import json
from experiment.dwarf_source import align_occurrence_sequences

fixture = json.load(open("experiment/dwarf-source/controlled-positive.json"))
