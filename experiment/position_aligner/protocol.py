...
POSITION_STATUSES = {"exact", "snapped", "interval", "ambiguous", "no_correspondence"}
...
class Position:
    ...
    def __post_init__(self) -> None:
        for name in ("build_id", "anchor_id", "event_phase"):
            _text(getattr(self, name), name)
        if self.event_phase not in EVENT_PHASES:
            raise ValueError(f"unknown event phase: {self.event_phase}")
...
class Correspondence:
    ...
    def __post_init__(self) -> None:
        ...
        if self.position_status in {"ambiguous", "no_correspondence", "interval"} and self.target is not None:
...
def validate_events(value: Sequence[ProgressEvent | Mapping[str, Any]], run: BuildRun | None = None) -> tuple[ProgressEvent, ...]:
    ...
    if run is not None:
        if any(event.build_id != run.build_id or event.run_id != run.run_id or event.event_phase != run.event_phase for event in events):
            raise ValueError("event does not match BuildRun identity or phase")
...
def validate_position(value: Position | Mapping[str, Any], run: BuildRun) -> Position:
    ...
    if position.build_id != run.build_id or position.event_phase != run.event_phase:
        raise ValueError("position does not match BuildRun identity or phase")
