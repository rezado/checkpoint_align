...
class PositionAligner:
    ...
    def align(self, source_trace, target_trace, correspondence, source_position, policy=None) -> AlignmentResult:
        ...
        canonical_source = tuple((event.semantic_key, event.occurrence, event.event_phase) for event in source_events)
        canonical_target = tuple((event.semantic_key, event.occurrence, event.event_phase) for event in target_events)
...
def _same_position(event: ProgressEvent, position: Position) -> bool:
    return (event.build_id, event.anchor_id, event.occurrence, event.event_phase) == (position.build_id, position.anchor_id, position.occurrence, position.event_phase)


def _position_from_event(event: ProgressEvent, run: BuildRun, requested: int | None) -> Position:
    if event.workload_icount is None:
        return Position(run.build_id, event.anchor_id, event.occurrence, event.event_phase, event.pc, None, None, None, None, None)
    interval, offset = divmod(event.workload_icount, run.interval_instructions)
    return Position(run.build_id, event.anchor_id, event.occurrence, event.event_phase, event.pc, event.workload_icount, interval, offset, event.workload_icount, 0)
