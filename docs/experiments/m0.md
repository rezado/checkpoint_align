...
并从已经存在的 `data/lbm` 和 `data/multi-workload` 日志/产物
...
```sh
python3 scripts/m0/derive_lbm_a20_evidence.py
python3 scripts/m0/validate_m0_fixtures.py
python3 -m json.tool data/fixtures/m0/schema_fixture.json >/dev/null
python3 -m json.tool data/fixtures/m0/gold-correspondence.json >/dev/null
```

`derive_lbm_a20_evidence.py` 的输出是
`data/fixtures/m0/lbm-a20-repeatability.json`；它重新计算日志字段、checkpoint
...
零基 interval 算术、运行级 event phase、typed reject、occurrence transform、
...
- fixture 的 run manifest 固定写出 `before_instruction`；event 和 position
  不再重复携带 phase。
- 位置身份是 `(build_id, anchor_id, occurrence)`。A/B 的
...
|---|---|---:|---:|---|---:|---|
| `legacy-rerun-20260828-125428` | `data/lbm/reproduce.sh` | 119,555,201 | 380,000,005 | `0x1312c` | 540,000,432 | `e8922837ff7ca6bb030249eaeec42d2f45784c82a17e1a2d2174095f26720fed` |
| `legacy-rerun-20260828-125533` | `data/lbm/reproduce.sh` | 119,548,301 | 380,000,000 | `0x13134` | 540,000,452 | `cab5de4c5279fc7229f1d5ca02e0dfd46711332d9f8b0a5244e1efa22c17e28a` |
...
| `generic-suite-lbm-A20-B20` | historical checkpoint driver | 119,460,626 | 380,000,000 | `0x11ac6` | 580,000,485 | `858a4102689fbfd9fab20091893e7751ccb92d1188a255559479f0df3ad75ad3` |
...
/usr/bin/time -f 'wall=%e exit=%x' timeout 120s stdbuf -oL -eL \
  ...
  2>data/lbm/restore-logs/B-point20-generate-final.err.log
```
...
```sh
timeout 60s stdbuf -oL -eL data/lbm/tools/riscv64-nemu-interpreter \
  -b -I 40000000 --cpt-restorer data/lbm/tools/gcpt.bin \
  --simpoint-profile --dont-skip-boot --cpt-interval 20000000 \
  -D data/lbm/post-restore-profile-A -w lbm -C post-restore \
  data/lbm/A/checkpoint/20/_20_0.000069_memory_.zstd

timeout 60s stdbuf -oL -eL data/lbm/tools/riscv64-nemu-interpreter \
  -b -I 40000000 --cpt-restorer data/lbm/tools/gcpt.bin \
  --simpoint-profile --dont-skip-boot --cpt-interval 20000000 \
  -D data/lbm/post-restore-profile-B -w lbm -C post-restore \
  data/lbm/generated-B-point20-final/aligned-point20/lbm/20/\
_20_1.000000_memory_.zstd
```

两次 legacy 运行及其生成参数仅作为历史 provenance 保存；原
`data/lbm/reproduce.sh` 已删除，新运行请使用动态 occurrence 流程。

### Generic suite

外层命令和重定向：

The generic-suite run below is historical provenance; its original driver has
been removed. Use `src/checkpoint_align/cli.py` for new runs.

该 wrapper 实际执行的 B 生成命令为（由脚本参数和日志共同核对）：

```sh
/nfs/home/wujiabin/work/checkpoint_align/data/multi-workload/tools/riscv64-nemu-interpreter \
  /nfs/home/wujiabin/work/checkpoint_align/data/multi-workload/workloads/lbm/B/bin/lbm.fw_payload.bin \
  -D /nfs/home/wujiabin/work/checkpoint_align/data/multi-workload/results/lbm/A20-B20/target-generation \
  -w lbm -C aligned -b -I 580000000 \
  -S /nfs/home/wujiabin/work/checkpoint_align/data/multi-workload/results/lbm/A20-B20/target-cluster \
