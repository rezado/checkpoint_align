...
复用 `src/checkpoint_align/dwarf_source.py` 的 catalog。Python 侧从 catalog 生成每个 build 的紧凑 watchlist：
...
新增目录建议为 `src/checkpoint_align/qemu_occurrence/`：
...
```text
src/checkpoint_align/qemu_occurrence/
  occurrence_plugin.c
  collect.py
  README.md
...
data/results/m3/runtime/<candidate>/
  source-window.json
  source-binding.json
