...
aligner: zero-based interval arithmetic, run-level event phase, typed rejects,
...
def validate_position(
    ...
    if "event_phase" in position:
        fail(f"{where}.event_phase: phase belongs to the run manifest")
    required = {
        "schema_version",
        "build_id",
        "run_id",
    ...
    if position["schema_version"] != 1:
        fail(f"{where}.schema_version: expected 1")
    for key in ("build_id", "run_id", "anchor_id"):
...
def validate_schema_fixture(schema: Mapping[str, Any]) -> int:
    ...
    if contract.get("event_phase") != "before_instruction":
        fail("schema fixture must freeze before-instruction run semantics")
    ...
    for index, case in enumerate(cases):
        ...
        if case["kind"] == "identity":
            ...
            for cindex, correspondence in enumerate(correspondences):
                ...
                for key in ("anchor_id", "occurrence"):
...
def main() -> None:
    ...
    parser.add_argument(
        "--directory",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "data" / "fixtures" / "m0",
