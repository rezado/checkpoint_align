...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "data" / "fixtures" / "m0" / "lbm-a20-repeatability.json",
    )
    args = parser.parse_args()
    root = args.root.resolve()

    legacy_nemu = "data/lbm/tools/riscv64-nemu-interpreter"
    generic_nemu = "data/multi-workload/tools/riscv64-nemu-interpreter"
    legacy_gcpt = "data/lbm/tools/gcpt.bin"
    generic_gcpt = "data/multi-workload/tools/gcpt.bin"
    old_checkpoint = "data/lbm/A/checkpoint/20/_20_0.000069_memory_.zstd"
    generic_checkpoint = (
        "data/multi-workload/workloads/lbm/A/checkpoint/20/"
    ...
    legacy_generation = (
        ...
        "riscv64-nemu-interpreter data/lbm/B/bin/lbm.fw_payload.bin "
        "-D data/lbm/generated-B-point20-final -w lbm -C aligned-point20 "
        "-b -I 540000000 -S data/lbm/B-aligned-cluster "
        "--cpt-interval 20000000 --warmup-interval 20000000 "
        "--checkpoint-format zstd"
    )
    generic_generation = (
        "/nfs/home/wujiabin/work/checkpoint_align/data/multi-workload/tools/"
        "riscv64-nemu-interpreter "
        "/nfs/home/wujiabin/work/checkpoint_align/data/multi-workload/workloads/"
        "lbm/B/bin/lbm.fw_payload.bin -D "
        "/nfs/home/wujiabin/work/checkpoint_align/data/multi-workload/results/"
        "lbm/A20-B20/target-generation -w lbm -C aligned -b -I 580000000 "
        "-S /nfs/home/wujiabin/work/checkpoint_align/data/multi-workload/"
        "results/lbm/A20-B20/target-cluster --cpt-interval 20000000 "
        "--warmup-interval 20000000 --checkpoint-format zstd"
    )
    legacy_restore = {
        side: (
            "timeout 60s stdbuf -oL -eL data/lbm/tools/"
            "riscv64-nemu-interpreter -b -I 40000000 --cpt-restorer "
            "data/lbm/tools/gcpt.bin --simpoint-profile --dont-skip-boot "
            "--cpt-interval 20000000 -D data/lbm/post-restore-profile-"
            f"{side} -w lbm -C post-restore "
            f"{checkpoint}"
        )
        for side, checkpoint in {
            "A": old_checkpoint,
            "B": "data/lbm/generated-B-point20-final/aligned-point20/lbm/20/"
    ...
    generic_restore = {
        "A": (
            "/nfs/home/wujiabin/work/checkpoint_align/data/multi-workload/tools/"
            "riscv64-nemu-interpreter -b -I 40000000 --cpt-restorer "
            "/nfs/home/wujiabin/work/checkpoint_align/data/multi-workload/tools/gcpt.bin "
    ...
    legacy_rerun_generation = {
        "legacy-rerun-20260828-125428": (
            "timeout 120s data/lbm/tools/riscv64-nemu-interpreter "
            "data/lbm/B/bin/lbm.fw_payload.bin -D "
            "data/lbm/rerun-20260828-125428 -w lbm -C aligned-point20 -b "
            "-I 540000000 -S data/lbm/B-aligned-cluster "
            "--cpt-interval 20000000 --warmup-interval 20000000 "
            "--checkpoint-format zstd > "
            "data/lbm/rerun-20260828-125428/generate.out.log 2> "
            "data/lbm/rerun-20260828-125428/generate.err.log"
        ),
        "legacy-rerun-20260828-125533": (
            "timeout 120s data/lbm/tools/riscv64-nemu-interpreter "
            "data/lbm/B/bin/lbm.fw_payload.bin -D "
            "data/lbm/rerun-20260828-125533 -w lbm -C aligned-point20 -b "
            "-I 540000000 -S data/lbm/B-aligned-cluster "
            "--cpt-interval 20000000 --warmup-interval 20000000 "
            "--checkpoint-format zstd > "
            "data/lbm/rerun-20260828-125533/generate.out.log 2> "
            "data/lbm/rerun-20260828-125533/generate.err.log"
        ),
    }
    legacy_rerun_restore = {
        run_id: (
            "timeout 60s data/lbm/tools/riscv64-nemu-interpreter -b "
            "-I 40000000 --cpt-restorer data/lbm/tools/gcpt.bin "
            f"{checkpoint} > data/lbm/{run_id}/restore.out.log 2> "
            f"data/lbm/{run_id}/restore.err.log"
        )
        for run_id, checkpoint in {
            "legacy-rerun-20260828-125428": (
                "data/lbm/rerun-20260828-125428/aligned-point20/lbm/20/"
                "_20_1.000000_memory_.zstd"
            ),
            "legacy-rerun-20260828-125533": (
                "data/lbm/rerun-20260828-125533/aligned-point20/lbm/20/"
                "_20_1.000000_memory_.zstd"
            ),
        }.items()
    }
    ...
    runs = [
        build_run(
            ...
            "legacy-rerun-20260828-125428",
            "legacy_reproduce_sh",
            "data/lbm/rerun-20260828-125428/generate.out.log",
            "data/lbm/rerun-20260828-125428/generate.err.log",
            "data/lbm/rerun-20260828-125428/aligned-point20/lbm/20/"
            "_20_1.000000_memory_.zstd",
            target_restore_log="data/lbm/rerun-20260828-125428/restore.out.log",
            driver_command="./data/lbm/reproduce.sh",
        ...
        build_run(
            ...
            "legacy-rerun-20260828-125533",
            "legacy_reproduce_sh",
            "data/lbm/rerun-20260828-125533/generate.out.log",
            "data/lbm/rerun-20260828-125533/generate.err.log",
            "data/lbm/rerun-20260828-125533/aligned-point20/lbm/20/"
            "_20_1.000000_memory_.zstd",
            target_restore_log="data/lbm/rerun-20260828-125533/restore.out.log",
            driver_command="./data/lbm/reproduce.sh",
        ...
        build_run(
            ...
            "legacy-final",
            "legacy_manual",
            "data/lbm/restore-logs/B-point20-generate-final.out.log",
            "data/lbm/restore-logs/B-point20-generate-final.err.log",
            "data/lbm/generated-B-point20-final/aligned-point20/lbm/20/"
            "_20_1.000000_memory_.zstd",
            source_profile="data/lbm/post-restore-profile-A/post-restore/lbm/simpoint_bbv.gz",
            target_profile="data/lbm/post-restore-profile-B/post-restore/lbm/simpoint_bbv.gz",
            source_restore_log="data/lbm/restore-logs/A-point20-post-profile.out.log",
            target_restore_log="data/lbm/restore-logs/B-point20-post-profile.out.log",
            result_json="data/lbm/experiment-result.json",
        ...
        build_run(
            ...
            "historical_checkpoint_cli",
            "data/multi-workload/results/lbm/A20-B20/logs/generate.out.log",
            "data/multi-workload/results/lbm/A20-B20/logs/generate.err.log",
            "data/multi-workload/results/lbm/A20-B20/target-generation/aligned/lbm/20/"
            "_20_1.000000_memory_.zstd",
            source_profile=(
                "data/multi-workload/results/lbm/A20-B20/source-restore/profile/"
                "post-restore/lbm/simpoint_bbv.gz"
            ),
            target_profile=(
                "data/multi-workload/results/lbm/A20-B20/target-restore/profile/"
                "post-restore/lbm/simpoint_bbv.gz"
            ),
            source_restore_log="data/multi-workload/results/lbm/A20-B20/"
            "source-restore/logs/restore.out.log",
            target_restore_log="data/multi-workload/results/lbm/A20-B20/"
            "target-restore/logs/restore.out.log",
            result_json="data/multi-workload/results/lbm/A20-B20/checkpoint-result.json",
            driver_command="historical checkpoint command recorded in the source artifact",
    ...
    common_paths = {
        "target_elf": (
            "data/lbm/B/elf/lbm.elf",
            "data/multi-workload/workloads/lbm/B/elf/lbm.elf",
        ),
        "target_firmware": (
            "data/lbm/B/bin/lbm.fw_payload.bin",
            "data/multi-workload/workloads/lbm/B/bin/lbm.fw_payload.bin",
        ),
        "target_run_script": (
            "data/lbm/B/cmd/lbm.run.sh",
            "data/multi-workload/workloads/lbm/B/cmd/lbm.run.sh",
        ),
        "target_json": (
            "data/lbm/B/json/lbm.json",
            "data/multi-workload/workloads/lbm/B/json/lbm.json",
        ),
        "target_simpoints": (
            "data/lbm/B/cluster/simpoints0",
            "data/multi-workload/workloads/lbm/B/cluster/simpoints0",
        ),
        "target_weights": (
            "data/lbm/B/cluster/weights0",
            "data/multi-workload/workloads/lbm/B/cluster/weights0",
        ),
        "target_profile": (
            "data/lbm/B/profiling/simpoint_bbv.gz",
            "data/multi-workload/workloads/lbm/B/profiling/simpoint_bbv.gz",
        ),
    ...
    report = {
        ...
        "baseline_inputs": {
            "source": build_side_artifacts(root, "data/lbm/A"),
            "target": build_side_artifacts(root, "data/lbm/B"),
